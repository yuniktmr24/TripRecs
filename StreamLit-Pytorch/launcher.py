...
def travel_fare_page():
    ...
    if location_query:
        ...
        query_url = f"http://localhost:8983/solr/Airfares/query?q=destination:\"{location_query}\""
        # Send a GET request to Solr
        response = requests.get(query_url)
        if response.status_code == 200:
            data = response.json()
            fare_data = data['response']['docs']
            if fare_data:
                st.write(f"### Travel fare results for: {location_query}")
                for item in fare_data:
                    with st.expander(f"{item['destination']} ({item['departure_city']} to {item['arrival_city']})"):
                        st.markdown("""
                            **Fare:** {:.2f} {}
                            **Airline:** {}
                            **Departure Date:** {}
                        ...
                        """.format(
                            item['fare_amount'], item['currency'],
                            item['airline'],
                            item['departure_date'],
                            item['return_date'],
                            item['flight_duration_hours']
                        ))
            else:
                st.write("No travel fares found for this location.")
        else:
            st.error("Failed to retrieve data from Solr. Please check the server connection and try again.")
    else:
        st.write("Please enter a destination to search for travel fares.")
